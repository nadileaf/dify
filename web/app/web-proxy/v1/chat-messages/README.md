...
```
其他项目后端 → /web-proxy/v1/chat-messages (Web 代理)
                     ↓
...
```bash
POST https://your-domain.com/web-proxy/v1/chat-messages
...
curl -X POST 'https://agent.nadileaf.com/web-proxy/v1/chat-messages' \
...
- ✅ **只需部署前端**：不需要部署后端
- ✅ **避免路由冲突**：使用 `/web-proxy/` 前缀，不与后端 `/api/` 冲突
...
# 改为
curl https://agent.nadileaf.com/web-proxy/v1/chat-messages \
  --header 'Authorization: Bearer app-xxx' \
